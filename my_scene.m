function [] = my_scene(voc_size,varargin)
...
if strcmp(poolerType,'hor')
	...
	poolStr = sprintf('%sh%dq%d',poolerType,numHorDivs,numQuadDivs);
end
...
switch spatialEmbedding
    case 0
        prms.codebook = fullfile(DataDir, sprintf('codebooks/%s_%d_%d.mat',prms.experiment.dict_suffix, voc_size, desc_dim)); % desired location of codebook
    otherwise
        prms.codebook = fullfile(DataDir, sprintf('codebooks/%s_%d_%d_spatial.mat',prms.experiment.dict_suffix, voc_size, desc_dim)); % desired location of codebook
end
prms.dimred = fullfile(DataDir, sprintf('dimred/%s_pca_%d.mat',prms.experiment.dict_suffix, pca_desc_dim)); % desired location of low-dim projection matrix
...
if bCrossValSVM
    ...
    c = [0.1 0.5 1 1.6 2.4 3.2 5 6 10];
    %c = [1.6];
    maxmap = 0;
    maxci = 1;
    for ci = 1:length(c)
